...
const app = express();
const PORT = 6161;
const WEG_API_URL = 'https://ecatalog.weg.net/tec_cat/tech_motor_sel_functions.asp';
...
const getEfficiencyAndId = async (cd_produto, poles, outputHP) => {
  ...
  try {
    ...
    if (data.motor && data.motor.length > 0) {
      ...
      console.log(`[DEBUG] Dados extraídos - cd_produto: ${cd_produto}, motorId: ${motorId}, efficiency: ${efficiency}, speed: ${speed}`);
...
app.listen(PORT, () => {
  ...
  console.log(`✓ API WEG: ${WEG_API_URL}`);
});
